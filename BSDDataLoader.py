...
def bsd500(dest="/home/td/Documents/salt/"):#自行修改路径！！
...
class DatasetFromFolder(data.Dataset):
    def __init__(self, image_dir, target_mode, colordim, input_transform=None, target_transform=None):
        ...
        self.image_filenames = [x for x in listdir( join(image_dir,'images') ) if is_image_file(x)]
    ...
    def __getitem__(self, index):
        print(index)
        input = load_img(join(self.image_dir,'images',self.image_filenames[index]),self.colordim)
        if self.target_mode=='seg':
            target = load_img(join(self.image_dir,'masks',self.image_filenames[index]),1)
        else:
            target = load_img(join(self.image_dir,'masks',self.image_filenames[index]),1)
